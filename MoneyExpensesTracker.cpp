...
void Income();
void Expenses();
string Prediction(string Category);
int first = 1;
...
vector<string> split(const string& str, char delimiter, int limit) {
	...
	int number = 0;
	while (number != limit && (end = str.find(delimiter, start)) != string::npos) {
...
void Edit() {
	...
	cout << "==================================================================================" << endl;
	cout << std::left;
	cout << setw(10) << "NO" << setw(20) << "Date" << setw(15) << "Amount" << setw(20) << "Category" << setw(20) << "Note" << "\n";
	...
	while (getline(file, Line)) {
		...
		// The array itself
		cout << std::left;
		cout << setw(10) << (Count + 1) << setw(20) << Date << setw(15) << Amount << setw(20) << Category << setw(20) << Note << "\n";
		Count++;
	}
	file.close();
	...
	while (getline(fileIn, line)) {
		if (currentRow == (rowNumber - 1)) {
			vector<string> fields = split(line, ' ', 3);
...
void Delete() {
	...
	{
		cout << "Line deleted suscessfully!\n";
	}
	...
	case 1:
	{
		...
		while (input >> DateStore >> Amount >> CategoryStore)
		{
			...
			if (Pass == 1)
			{
				//Display the income
				cout << std::left;
				cout << setw(10) << (Count + 1) << setw(20) << DateStore << setw(15) << Amount << setw(20) << Category << setw(20) << Note << "\n";
				Count++;
				Pass = 0;
			}
		}
...
string Prediction(string Category)
{
	...
	case 1:
	{
		...
		average = sum / i;
		cout << "\nDaily " << Category << " in month (" << SMonth << "): RM" << std::fixed << std::setprecision(2) << average << "\n\n";
	...
	case 2:
	{
		...
		for (int k = 1; k < 13; k++)
		{
			cout << "Month " << Category << " (" << k << "): RM" << monthly[k] << endl;
			sum += monthly[k];
		}

		average = sum / 12;
		cout << "\nMonthly " << Category << " (" << SYear << "): RM" << std::fixed << std::setprecision(2) << average << "\n\n";
	...
	case 3:
	{
		...
		average = sum / (range + 1);
		cout << "\nAnnually " << Category << " (" << Year1 << " to " << Year2 << "): RM" << std::fixed << std::setprecision(2) << average << "\n\n";
